...
os.environ['PYGAME_HIDE_SUPPORT_PROMPT'] = "1"                                      # Ocultar el Mensaje de Bienvenida de Pygame

# Importar e Inicializar Pygame
import pygame                                                                       # Módulo para Desarrollo de videojuegos
pygame.init()                                                                       # Inicialización de Pygame
pygame.mixer.init()                                                                 # Inicialización del Módulo de Sonido de Pygame

# Demás Módulos
from settings.constants import *                                                    # Importar Constantes del Juego
from utils.colors import *                                                          # Importar Colores del Juego 
from utils.paths import get_image_path, get_music_path                              # Función para obtener Rutas de Imágenes o Música
from items.card import Card                                                         # Clase para manejar las Tarjetas del Juego
...
pygame.mixer.music.load(background_music)                                           # Cargar Música de Fondo
pygame.mixer.music.set_volume(0.3)                                                  # Volumen más Bajo (30%)

# Cargar Efectos de Sonido
point_sound = pygame.mixer.Sound(get_music_path("Point.wav"))                       # Sonido al Emparejar Tarjetas
point_sound.set_volume(0.7)                                                         # Ajuste de Volumen del Sonido de Punto
error_sound = pygame.mixer.Sound(get_music_path("Error.wav"))                       # Sonido al No Emparejar Tarjetas
error_sound.set_volume(0.7)                                                         # Ajuste de Volumen del Sonido de Error
...
cards = [
    ...
    [
        ...
        Card(get_image_path("PSG.png")), 
        Card(get_image_path("Madrid.png")), 
        Card(get_image_path("Barca.png")), 
        Card(get_image_path("Inter.png"))
...
button = pygame.Rect(0, SCREEN_HEIGHT - BUTTON_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT)
...
def win() -> bool:
    ...
    """
       ...
       -  Argumentos: Ninguno
       -  Retorno: Ninguno
       -  Objetivo Principal: Verificar si todas las Tarjetas están Destapadas
    """

    # Verifica si todas las Tarjetas han sido Descubiertas
    for row in cards:                                                               # Itera sobre cada Fila de Tarjetas
        for card in row:                                                            # Itera sobre cada Tarjeta en la Fila
            if not card.discovered: return False                                    # Si alguna Tarjeta no está Descubierta, False
    return True                                                                     # Si todas las Tarjetas están Descubiertas, True
...
while True:
    ...
    game_screen.fill(WHITE)                                                         # Renderizado del Fondo
    ...
    for row_index, row in enumerate(cards):                                         # Itera sobre cada Fila de Tarjetas
        for card_index, card in enumerate(row):                                     # Itera sobre cada Tarjeta en la Fila
            card_pos = (card_index * CARD_SIZE, row_index * CARD_SIZE)              # Posición de la Tarjeta
            if card.show or card.discovered:                                        # Si la Tarjeta está boca arriba o ha sido Emparejada
                game_screen.blit(                                                   # Dibuja la Imagen de la Tarjeta
                ...
                pygame.draw.rect(                                                   # Dibuja el Reverso de la Tarjeta
                    game_screen, 
                    GRAY, 
                    (*card_pos, CARD_SIZE, CARD_SIZE)
                    
                ) 
            pygame.draw.rect(                                                       # Dibuja el Borde de la Tarjeta
    ...
    pygame.draw.rect(game_screen, WHITE if game_started else MARINE_BLUE, button)   # Dibuja el Botón
    game_screen.blit(                                                               # Dibuja el Texto del Botón
        
        FONT.render("Iniciar Juego", True, GRAY if game_started else WHITE), 
        (X_FONT, Y_FONT)
    ...
    for event in pygame.event.get():                                                # Itera sobre cada Evento en la Cola de Eventos
        if event.type == pygame.QUIT:                                               # Si el Evento es de Salida
            ...
            if button.collidepoint(event.pos):                                      # Si el Clic está dentro del Botón
            ...
            if card.show or card.discovered: continue                               # Si la Tarjeta ya está boca arriba o emparejada, ignora Clic
            ...
            if x1 is None and y1 is None:                                           # Primera Tarjeta seleccionada
                ...
                if card1.image_path == card2.image_path:                            # Si las Tarjetas coinciden
                    ...
                    error_sound.play()                                              # Reproduce el Sonido de Error
